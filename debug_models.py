#!/usr/bin/env python
import sys, os, signal, traceback, faulthandler, logging
faulthandler.enable(file=sys.stderr)
logging.basicConfig(level=logging.DEBUG, format='[%(asctime)s] [%(levelname)s] %(message)s',
    handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler('/tmp/model_debug.log', mode='w')])
...
def signal_handler(signum, frame):
    ...
    safe_log(f"Signal: {signum}", 'critical')
...
try:
    with app.app_context():
        safe_log("DEBUG: Inside app context")
